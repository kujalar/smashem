var barrack = {  //namespace barrack
  ...
  getOrc: function() {
    return barrack.buildMonster(monsterManual.orc);
  },
  ...
  buildMonster: function(monster) {
    ...
    var attackList = monster.attackList;
    var monsterActions = [];
    for(var i=0; i < attackList.length;i++) {
      var action = attackList[i];
      var charAction = {name:action.name,toHit:action.toHit,hit:action.hit,critHit:action.critHit,selected:false};
      monsterActions.push(charAction);
    }
